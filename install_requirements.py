...
class TestJapanera(unittest.TestCase):
    ...
    def test_strptime_first_year(self):
        actual = self.japera.strptime("令和元年五月十九日", "%-E%-kO年%-km月%-kd日")
        self.assertEqual(actual, [datetime(2019, 5, 19, 0, 0)])


if __name__ == "__main__":
    print(Japanera().strptime("令和元年五月十九日", "%-E%-kO年%-km月%-kd日"))
    era = Japanera().era(datetime(2019, 5, 19, 0, 0))
    print(era)
    unittest.main()
